...
function dvwaHtmlEcho( $pPage ) {
	...
	if( $pPage[ 'info_button' ] ) {
		$systemInfoHtml = dvwaButtonInfoHtmlGet( $pPage[ 'info_button' ] ) . " $systemInfoHtml";
	}
...
function dvwaHtmlEchoSQL( $pPage ) {
	$menuBlocks = array();
	$menuBlocks[ 'home' ] = array();
	$menuHtml = '';
	...
	$userInfoHtml = '<em class="form__normal-text">Username:</em> ' . ( dvwaCurrentUser() );
	$securityLevelHtml = "<em class=\"form__normal-text\">Security Level:</em> {$securityLevelHtml}";
	...
	$systemInfoHtml = "";
	if( dvwaIsLoggedIn() ) 
		$systemInfoHtml = "<div align=\"right\">{$userInfoHtml}<br />{$securityLevelHtml}</div>";
	if( $pPage[ 'source_button' ] ) {
		$systemInfoHtml = dvwaButtonSourceHtmlGet( $pPage[ 'source_button' ] ) . " $systemInfoHtml";
	}
	if( $pPage[ 'help_button' ] ) {
...
function dvwaButtonInfoHtmlGet( $pId ) {
	return "<input type=\"button\" value=\"View Info\" class=\"popup_button\" id='info_button' data-info-url='" . DVWA_WEB_PAGE_TO_ROOT . "vulnerabilities/sqlinfo.php' )\">";
}
